...
exports.formatInterviewPrep = formatInterviewPrep;
const groq_sdk_1 = __importDefault(require("groq-sdk"));
const groq = new groq_sdk_1.default({ apiKey: process.env.GROQ_API_KEY });
async function generateConversationalResponse(userMessage, jd, cvText, chatHistory) {
...
async function generateInterviewQuestions(jd, cvText) {
    const response = await groq.chat.completions.create({
        model: 'llama-3.3-70b-versatile',
        messages: [
            {
...
function formatInterviewPrep(questions) {
